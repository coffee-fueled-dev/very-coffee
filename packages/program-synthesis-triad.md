...
The execution of the synthesized plan $\Pi^{\ast}$ closes the self-improving loop of the system. The sequence of $\mathsf{MacroAction}\text{s}$ in $\Pi^{\ast}$ is unrolled into its full OBP $\mathsf{Action}$ trace $\mathrm{Tr}(\Pi^{\ast})$, which serves as the template for subsequent executions, providing new, optimized data for the TKN Online Learner.
