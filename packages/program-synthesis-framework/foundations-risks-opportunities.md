...
> **Remark (Ports as neural fields / adapters).** Informally, one can view each port $P$ as defining a _local field_ over a global learned world-model: the cone $P(x, C)$ plays the role of a localized conditional distribution over trajectories, analogous to a neural field centered on a particular region of state–context space. In implementation terms, a port-specific conditioning mechanism (e.g.\ adapter modules such as LoRA, routing/gating keyed by Port embeddings, or attention biases) can specialize a large neural world-model to behave consistently with the admissible trajectories in $P(x, C)$, allowing macro-actions to “unwind” learned behaviors along previously successful causal paths. These mechanisms do not constitute part of the port’s formal definition; they are merely implementation details for realizing the stochastic action cone at scale.
