...
The tkn algorithm reads a sequence of inputs one at a time, discovering and segmenting patterns as they emerge. It uses a LZ-style inclusion heuristic to decide whether to grow or emit sequences:
...
This said, the tkn algorithm is not a tokenizer on its own. Its focuses is greedy, fast, pattern discovery rather than optimal segmentation. As patterns are emit, the probability of emission of meaningful pattern will be higher than the probability of a non-meaningful pattern. In the _global_ context, the emission frequencies and connectedness to other patterns in the graph of pattern co-occurences encodes the structure of the data. This can then _be used_ for tasks such as tokenization.
